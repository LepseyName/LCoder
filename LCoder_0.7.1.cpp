...
char //*KEY=new char[KEY_size],                   					//Г‘Г Г¬ Г±ГҐГЄГ°ГҐГІГ­Г»Г© ГЄГ®Г¤
	std_exten[]=".sec",												//Г‘ГІГ Г­Г¤Г Г°ГІГ­Г®ГҐ Г°Г Г±ГёГЁГ°ГҐГ­ГЁГҐ 
	std_enter[]="0";												//Г‚ГўГ®Г¤ Г±Г¤Г Г­ГІГ Г°Г­Г®ГЈГ® Г°Г Г±ГёГЁГ°ГҐГ­ГЁГї

char shifr(char ,unsigned char , unsigned char , unsigned char , unsigned char , unsigned char ,bool );
char menu();
char* KEY_check (char*, bool);
char* KEY_enter (char );
char* KEY_revers(char*);
void cin_clear();
...
int main(int argc, char* argv[]){
    ...
    while(1){																								//Открытие выходного файла
		...
		if (strcmp(name_out,std_enter)==0) {																//Проверяем имя на стандартное
    ...
    if(!KEY){																								//Если ключа нет или он не правильный
		...
		if(men=='2'){																						//Если декодировать  
	    	...
	    	KEY = KEY_revers(KEY);																			//и его немного изменить
	    }
...
char* KEY_check(char* KEY, bool print = true){
	if(KEY){
		...
		if( len % (KEY_SIZE/5)!=0 || len<1 || len>KEY_SIZE){
			if(print) cout<<"Invalide KEY! Must be ...\n";
			return 0;
		}
	}
	return KEY;
...
char* KEY_revers(char *KEY){																				//Функция разварота ключа
	char *buf = new char[strlen(KEY) + 1],																		//Создаем новый ключ и ссылку на его голову
		 *revers_KEY=buf;	
	for(int i=strlen(KEY)-1;i>=3;i-=4){																		//Меняем символы четверками
		*(buf++)=KEY[i-3];
		*(buf++)=KEY[i-2];
		*(buf++)=KEY[i-1];
		*(buf++)=KEY[i-0];
